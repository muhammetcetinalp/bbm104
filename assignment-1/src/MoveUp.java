...
public class MoveUp {
    public static void move_up(int number,int row,HashMap<Integer,String> dict,int total,HashMap<String,Integer> dictForLocation,int column){
        if (number <= row) {
            int temporay = number;
            ...
            if (number >= row*(column - 1) && dict.get(number - row).equals("W")){
                int temp = 0;
